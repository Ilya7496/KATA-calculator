...
public class Main {
    ...
    public static String calculator(String input) throws Exception {
        ...
        if (numbers.length > 3) {
            throw new Exception("Формат операции не удовлетворяет заданию - два операнда и один оператор(+,-,/,*)");
        }
        if (numbers.length < 3) {
        ...
        if (a > 10 || b > 10) {
            throw new Exception("Введеные числа должны быть меньше 10");
        }
        switch (x) {
            case "+" -> {
                res = a + b;
            ...
            case "*" -> {
                ...
                if (control == 2) {
                    if (res <= 0) {
                        throw new Exception("В римсокй системе счисления нет отрицательных чисел и нуля");
                    } else {
                        input = rim[res - 1];
                        System.out.println(input);
                    }
            ...
            case "/" -> {
                ...
                if (control == 2) {
                    if (res <= 0) {
                        throw new Exception("В римской системе счисления нет отрицательных числе и нуля");
                    } else {
                        input = rim[res - 1];
                        System.out.println(input);
                    }
            ...
            default -> throw new Exception("Неверный формат ввода");
        }
        return input;
    }
}